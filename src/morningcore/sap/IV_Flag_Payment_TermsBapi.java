...
public class IV_Flag_Payment_TermsBapi extends BaseBean{
    ...
    public void getData(){
        ...
        String modeid = getModeId("uf_fktj");
        ...
        try{
            ...
            paraList.setValue("X","IV_FLAG_PAYMENT_TERMS");
            writeLog("paraList--> " + paraList.toString());
            sapconnection.execute(jcoFunction);
            Table outtab = jcoFunction.getExportParameterList().getTable("ET_PAYMENT_TERMS");
            //RecordSet rs2 = new RecordSet();
//            writeLog("outtab--> " + outtab.toString());
            String PYMT_TERMS = "";
            String PYMT_TERMS_DESC = "";
            for(int j = 0;j < outtab.getNumRows();j++){
                outtab.setRow(j);
                PYMT_TERMS = Util.null2String((String)outtab.getValue("PYMT_TERMS"));
                PYMT_TERMS_DESC = Util.null2String((String)outtab.getValue("PYMT_TERMS_DESC"));
                writeLog("PYMT_TERMS--> " + PYMT_TERMS + ",PYMT_TERMS_DESC--> " + PYMT_TERMS_DESC);
                int no = 0;
                String sql = "select count(1) as no from uf_fktj where PYMT_TERMS = '" + PYMT_TERMS + "'";
                rs.execute(sql);
                if(rs.next()){
                    no = rs.getInt("no");
                }
                ...
                map.put("PYMT_TERMS", PYMT_TERMS);
                map.put("PYMT_TERMS_DESC", PYMT_TERMS_DESC);
                ...
                writeLog("map:" + map);
                if(no>0){
                    sql = "update uf_fktj set PYMT_TERMS_DESC = '" + PYMT_TERMS_DESC + "' where  PYMT_TERMS = '" + PYMT_TERMS + "'";
                    rs.execute(sql);
                    writeLog("sql-->" + sql);
                }else{
                    result = iu.insert(map, "uf_fktj");
                    sql = "select id from uf_fktj where  PYMT_TERMS = '" + PYMT_TERMS + "'";
                    rs.execute(sql);
                    if (rs.next()) {
