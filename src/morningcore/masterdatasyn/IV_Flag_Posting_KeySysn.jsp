...
<%@ page import="morningcore.sap.IV_Flag_Posting_KeyBapi" %>
...
<FORM id=report name=report method=post> <!--/task/point/sysnDevelopPerPoint.jsp-->
    ...
    <%
        if("1".equals(multiIds)){
            IV_Flag_Posting_KeyBapi IV_Flag_Posting_KeyBapi = new IV_Flag_Posting_KeyBapi();
            IV_Flag_Posting_KeyBapi.getData();
            out.println("<font size=\"5px\" color=\"red\">执行完成，获取成功!</font>");
        }
    %>

    <wea:layout type="2col">
        <wea:group context="过账码同步" attributes="test">
            <wea:item>过账码同步</wea:item>
            <wea:item>
                <input name="hi2" type="button" value="过账码同步" class="e8_btn_top_first"
