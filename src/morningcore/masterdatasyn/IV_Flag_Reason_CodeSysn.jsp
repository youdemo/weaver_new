...
<%@ page import="morningcore.sap.IV_Flag_Reason_Codeapi" %>
...
<FORM id=report name=report method=post> <!--/task/point/sysnDevelopPerPoint.jsp-->
    ...
    <%
        if("1".equals(multiIds)){
            IV_Flag_Reason_Codeapi IV_Flag_Reason_Codeapi = new IV_Flag_Reason_Codeapi();
            IV_Flag_Reason_Codeapi.getData();
            out.println("<font size=\"5px\" color=\"red\">执行完成，获取成功!</font>");
        }
    %>

    <wea:layout type="2col">
        <wea:group context="原因代码同步" attributes="test">
            <wea:item>原因代码同步</wea:item>
            <wea:item>
                <input name="hi2" type="button" value="原因代码同步" class="e8_btn_top_first"
