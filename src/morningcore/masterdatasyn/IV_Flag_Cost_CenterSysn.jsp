...
<%@ page import="morningcore.sap.IV_Flag_Cost_CenterBapi" %>
...
<FORM id=report name=report method=post> <!--/task/point/sysnDevelopPerPoint.jsp-->
    ...
    <%
        if("1".equals(multiIds)){
            IV_Flag_Cost_CenterBapi IV_Flag_Cost_CenterBapi = new IV_Flag_Cost_CenterBapi();
            IV_Flag_Cost_CenterBapi.getData();
            out.println("<font size=\"5px\" color=\"red\">执行完成，获取成功!</font>");
        }
    %>

    <wea:layout type="2col">
        <wea:group context="成本中心同步" attributes="test">
            <wea:item>成本中心同步</wea:item>
            <wea:item>
                <input name="hi2" type="button" value="成本中心同步" class="e8_btn_top_first"
