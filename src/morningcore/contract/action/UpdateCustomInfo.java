...
public class UpdateCustomInfo implements Action{

	@Override
	public String execute(RequestInfo info) {
		BaseBean log = new BaseBean();
		log.writeLog("开发插入历史记录");
		...
		if(rs.next()) {
			...
			xzkhmc = Util.null2String(rs.getString("xzkhmc"));	
		}
		...
		while(rs.next()) {
			...
			sql_dt = "update uf_khzsj set "+dbfieldname+"=(select newvalue from uf_change_detail where seqno='"+seqno+"') where id="+xzkhmc;
