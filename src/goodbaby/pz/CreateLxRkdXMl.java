...
public class CreateLxRkdXMl {
	public String CreateXML(String begindate,String enddate,String yjcbzx,String scr,String rqids) {
		...
		int entryid = 1;
		sql="select a.shck,sum(b.wsje*c.HL) as rmbje,(select CKMC from uf_stocks where id=a.shck) as shckmc  from "+tablename_rk+" a,"+tablename_rk+"_dt1 b ,"+tablename_dd+" c" + 
				" where a.id=b.mainid  and b.cgsqd = c. requestid and a.requestid in("+rqids+")" + 
				" group by a.shck";
		rs.executeSql(sql);
		while(rs.next()) {		
			String shckmc = Util.null2String(rs.getString("shckmc"));
			...
			xml.append("<abstract>"+yjcbzx+begindate.substring(0,4)+begindate.substring(5,7)+"NPP零星材料入库暂估"+shckmc+"</abstract>").append("\n");
			...
			Map<String, String> fzhsMap = xu.getFZHSMap(fyrzkm, "0");
		...
		//贷
		String rmbze = "";
		sql = "select sum(b.wsje*c.HL) as rmbje from "+tablename_rk+" a,"+tablename_rk+"_dt1 b ,"+tablename_dd+" c " + 
				"where a.id=b.mainid  and b.cgsqd = c. requestid and a.requestid in("+rqids+")";
		rs.executeSql(sql);
		if(rs.next()) {
		...
		xml.append("<entry>").append("\n");
		xml.append("<entry_id>"+entryid+"</entry_id> ").append("\n");
		xml.append("<account_code>220205010206</account_code>").append("\n");
		xml.append("<abstract>"+yjcbzx+begindate.substring(0,4)+begindate.substring(5,7)+"NPP零星材料入库暂估</abstract>").append("\n");
		...
		xml.append("<bill_date />").append("\n");
		xml.append("<auxiliary_accounting>").append("\n");
		Map<String, String> fzhsMap = xu.getFZHSMap("220205010206", "0");
		if(!"".equals(fzhsMap.get("wxsybfzhs"))&&!"1".equals(fzhsMap.get("wxsybfzhs"))) {
			if(!"".equals(xu.getWxsybfzhs(yjcbzx, fzhsMap.get("wxsybfzhs")))) {
				xml.append("<item name=\"外销事业部辅助核算\">"+xu.getWxsybfzhs(yjcbzx, fzhsMap.get("wxsybfzhs"))+"</item>").append("\n");
			}
