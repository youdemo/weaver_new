...
public class AutoRequestService extends BaseBean {
	 ...
	 * @param strJson
	 *            格式说明 ：
	 *            例子：{"DETAILS":{"DT1":[{"b1":"2","a1":"1"},{"b1":"22","a1"
	 *            :"12"}
	 *            ],"DT2":[{"b2":"22","a2":"11"}]},"HEADER":{"aa":"aa","bb"
	 *            :"bb"}} HEADER：存放主表信息 DETAILS存放明细表信息 明细1 以DT1标识的array
	 *            明细2以DT2标识 的array以此类推
	 * @param createrid
	 *            流程创建人id
	 * @param isNext
	 *            是否流转到下一节点 1 是 0 不是
	...
	public String createRequest(String workflowCode, String strJson,
		...
		if (creater.length() > 0 && !"1".equals(creater)) {
			sql = "select count(1) as count_cc from crm_customerinfo "
					+ "where id='" + creater;
			rs.executeSql(sql);
			if (rs.next()) {
		...
		if (creater.length() < 1) {
			retMap.put("MSG_TYPE", "E");
			retMap.put("MSG_CONTENT", "系统客户人员无法匹配！");
			retMap.put("OA_ID", "0");
