...
//		}
		sql = "insert into uf_kq_ldk_mid(ryid,rq,type) select id,'"+checkDate+"','0' from hrmresource where id not in(select distinct userid from HRMSCHEDULESIGN where signDate ='"+checkDate+"' and signTime>='00:00:00' and signTime<='12:00:00') and status<5 and nvl(belongto,-1)<=0 and (id in(select distinct xm from uf_tsmygskqry) or id in(select distinct xm from uf_jtzbkqry))";
		rs.executeSql(sql);
		//下班打卡
...
//		}
		...
		sql = "insert into uf_kq_ldk_mid(ryid,rq,type) select id,'"+checkDate+"','1' from hrmresource where id not in(select distinct userid from HRMSCHEDULESIGN where signDate ='"+checkDate+"' and signTime>='13:30:00' and signTime<='23:59:59') and status<5 and nvl(belongto,-1)<=0 and (id in(select distinct xm from uf_tsmygskqry) or id in(select distinct xm from uf_jtzbkqry))";
		rs.executeSql(sql);
	}
	...
	public void insertQqDateByDate(String checkDate) {
		...
		String checkNextDate = TimeUtil.dateAdd(checkDate, 1);
		sql = "select id from hrmresource a where status<5 and nvl(belongto,-1)<=0 and (id in(select distinct xm from uf_tsmygskqry) or id in(select distinct xm from uf_jtzbkqry)) and exists(select 1 from  HRMSCHEDULESIGN where userId=a.id and signDate='"+checkDate+"' and signTime>='08:46:00' and signTime<='12:00:00') and not exists(select 1 from  HRMSCHEDULESIGN where userId=a.id and signDate='"+checkDate+"' and signTime>='00:00:00' and signTime<'08:46:00')";
		rs.executeSql(sql);
		while(rs.next()) {
			String ryid = Util.null2String(rs.getString("id"));
			String signTime = "";
			sql_dt = "select min(signTime) as signtime from  HRMSCHEDULESIGN where  signDate='"+checkDate+"' and signTime>='08:46:00' and signTime<='12:00:00' and userId='"+ryid+"'";
			rs_dt.executeSql(sql_dt);
			if(rs_dt.next()) {
				signTime = Util.null2String(rs_dt.getString("signtime"));
				if(!"".equals(signTime)) {
			...
			sql_dt = "insert into uf_kq_qq_mid(ryid,rq,type,qqsj) values('"+ryid+"','"+checkDate+"','0','"+signTime+"')";
			rs_dt.executeSql(sql_dt);
		}
		//下班打卡
		...
		sql = "select id from hrmresource a where status<5 and nvl(belongto,-1)<=0 and (id in(select distinct xm from uf_tsmygskqry) or id in(select distinct xm from uf_jtzbkqry)) and exists(select 1 from  HRMSCHEDULESIGN where userId=a.id and signDate='"+checkDate+"' and signTime>='13:30:00' and signTime<'17:15:00') and not exists(select 1 from  HRMSCHEDULESIGN where userId=a.id and signDate='"+checkDate+"' and signTime>='17:15:00' and signTime<='23:59:59')";
		rs.executeSql(sql);
		while(rs.next()) {
			String ryid = Util.null2String(rs.getString("id"));
			String signTime = "";
			sql_dt = "select max(signTime) as signtime from  HRMSCHEDULESIGN where  signDate='"+checkDate+"' and signTime>='13:30:00' and signTime<='17:15:00' and userId='"+ryid+"'";
			rs_dt.executeSql(sql_dt);
			if(rs_dt.next()) {
				signTime = Util.null2String(rs_dt.getString("signtime"));
				if(!"".equals(signTime)) {
			...
			sql_dt = "insert into uf_kq_qq_mid(ryid,rq,type,qqsj) values('"+ryid+"','"+checkDate+"','1','"+signTime+"')";
			rs_dt.executeSql(sql_dt);
		}
	}
