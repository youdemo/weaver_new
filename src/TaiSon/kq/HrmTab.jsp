...
<%
	...
	if(_fromURL.equals("SystemRightGroup")){
		...
		if(cmd.equals("schedulediff")){//考勤相关
			...
			response.sendRedirect("/TaiSon/kq/kqReportUrl.jsp");
			return;
			...
			response.sendRedirect("/TaiSon/kq/CalendarStatementURL.jsp");
			return;
		}
	  ...
	  //String companyid=Util.null2String(request.getParameter("id"));
       ...
       return ;
		...
		if(AppDetachComInfo.isUseAppDetach()){
			Map resourceDetach = AppDetachComInfo.getAllDetachResourceInfo(user.getUID()+"");
			if(resourceDetach != null && !resourceDetach.isEmpty()){
				if(resourceDetach.get(id) == null) isApp = false;
			}
			if(!isApp){
		     ...
		     <%if(hasTree){%>
		       ...
		       String resourceid = Util.null2String(request.getParameter("resourceid"));
		        ...
		        <%
						...
						String subcompanyid=Util.null2String(request.getParameter("subcompanyid"));
						String departmentid=Util.null2String(request.getParameter("departmentid"));
						tab.add(new TabLi("/hrm/schedule/PSLManagementView.jsp?companyid="+companyid+"&subCompanyId="+subcompanyid+"&departmentid="+departmentid+"&cmd="+cmd,SystemEnv.getHtmlLabelName(33612,user.getLanguage()),true));
						out.println(tab.show());
					} else if(_fromURL.equals("HrmResourcePassword")){
						String id = Util.null2String(request.getParameter("id"));
