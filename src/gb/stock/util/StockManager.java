...
public class StockManager {
	...
	private boolean intoStock(Stock sk){
		String sql = "insert into uf_Stock(lch,dtid,wlmc,poid,rkrq,rksj,CKID,GYS,rkjg,wsdj,rate,thbs,rksl,ylysl,rkly,glrk) values('"
			+ sk.getRequestID() + "','" + sk.getDtID() + "','" + sk.getMaterielID() + "','" + sk.getPoid()
			+ "','" + sk.getcDate() + "','" + sk.getcTime() + "','" + sk.getCkid() + "','" + sk.getGys()
			+ "'," + sk.getUnitPrice() + ","+sk.getWsdj()+",'"+sk.getRate()+"','"+sk.getThbs()+"'," + sk.getcNum() + "," + sk.getfNum() + "," + sk.getType();
