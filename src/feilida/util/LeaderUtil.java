...
public class LeaderUtil implements Action {
    ...
    public String execute(RequestInfo info) {
        ...
        if (!"".equals(tableName)) {
            ...
            sql = " select ltrim(sys_connect_by_path(id,'/'),'/') as leadergroup from hrmresource where CONNECT_BY_ISLEAF=1  start with id= " + wfcreater + " connect by prior managerid=id ";
            log.writeLog("sql_leader=" + sql);
            rs.execute(sql);
            if (rs.next()) {
                leadergroup = Util.null2String(rs.getString("leadergroup")).split("/");
                for (int i = 0; i < leadergroup.length-1; i++) {
                    int j = i+1;
                    String tmpLeader = leadergroup[j];
