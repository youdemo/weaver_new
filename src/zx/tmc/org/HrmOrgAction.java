...
//		}
	...
	public ReturnInfo operResource(HrmResourceBean hrb){
		...
		RecordSet rs = new RecordSet();
		String sql = "";
		if(hrb.getBelongIdOrCode()>=0) {
			sql = "select id from hrmresource where workcode='"+workcode+"' and jobtitle='"+hrb.getJobtitle()+"' ";
		}else {
			sql = "select id from hrmresource where workcode='"+workcode+"' and (isnull(belongto,0)<=0 or belongto ='')";
		}
		rs.executeSql(sql);
		int id = 0;
		if(rs.next()){
		...
		if(belongFlag >= 0){
			if(belongFlag == 0){
				...
				sql = "select id from hrmresource where workcode='"+hrb.getBelongtoCode()+"' and (isnull(belongto,0)<=0 or belongto ='')";
				rs.executeSql(sql);
				if(rs.next()){
...
//			else s = s+1;
			...
			rs.executeSql("select id from hrmresource where workcode='"+workcode+"' and jobtitle='"+jobTitleID+"' order by id desc");
			if(rs.next()){
				id = rs.getInt("id");
			}
			if(id < 1){
