...
public class TestDemo2 {
	...
	public static void modifyDocumentAndSave() throws Exception {
		...
		bookMarkMap.put("定制方", "昆山泛微昆山泛微昆山泛微昆山泛微昆山泛微昆山泛微昆山泛微昆山泛微");
		bookMarkMap.put("承揽方", "张三");
		...
		if (this_book_list.getLength() != 0) {
			for (int j = 0; j < this_book_list.getLength(); j++) {
				...
				if("明细表".equals(bookMarkName)){
					System.out.println("aaa");
					child1 = (Element)oldBookStart.getParentNode().getNextSibling();
					System.out.println(child1.getAttribute("w:rsidR"));
				}
				if("明细表2".equals(bookMarkName)){
					System.out.println("bbb");
					child2 = (Element)oldBookStart.getParentNode();
					System.out.println(child2.getAttribute("w:rsidR"));
				}
...
//			}
		...
		while (entriesIter.hasMoreElements()) {
			...
			if (entry.getName().equals("word/document.xml")) {
				...
				//byte[] data = new byte[10240 * 512];
				//int readCount = incoming.read(data, 0, (int) entry.getSize());
				//System.out.println(entry.getName()+" "+(int) entry.getSize()+" "+readCount);
				docxOutFile.putNextEntry(new ZipEntry(entry.getName()));
	...
	public static void copy(Element child1,Element child2){
		...
		for(int j=0;j<5;j++){
			if(j==0){
				...
				Element clone=(Element)child1.getNextSibling();
     			NodeList wtList=clone.getElementsByTagName("w:t");
//				root.insertBefore(child1.cloneNode(true), child2);
//				Element clone=(Element)child2.getPreviousSibling();
//				NodeList wtList=clone.getElementsByTagName("w:t");
