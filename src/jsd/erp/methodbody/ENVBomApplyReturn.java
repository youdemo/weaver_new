...
/**
 * @author 作者  张瑞坤
 * @version 创建时间：2019-5-14 下午2:38:58
 *  加工件验证需求 回传
 */
public class ENVBomApplyReturn implements Action{
...
//				head.put("F_Status",Util.null2String(rs.getString("F_Status")));// 状态
		...
		log.writeLog("JSOn----JST-ENV-001--"+json.toString());
		String result = "";
		try {
			result = ts.serviceImp(json.toString(), "JST-ENV-001");
			log.writeLog("result----JST-ENV-001--"+result);
		} catch (Exception e) {
