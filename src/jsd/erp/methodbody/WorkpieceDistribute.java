...
/**
 * @author 作者  张瑞坤
 * @version 创建时间：2019-6-7 下午9:53:54
 * 加工件分发
 */
public class WorkpieceDistribute {
	public String triggerProcess(String jsonstr,String flag){
		...
		String creatercode = "";
		String result = "";
		String workflowid = "198";//Util.null2o(weaver.file.Prop.getPropValue("workflowids", flag));//"215";//流程id------------------
...
//						head.put("cjrbm", departmentid);
				...
				if(!head.isNull("FBuyerID")){//采购员
					String FBuyerID = head.getString("FBuyerID");
					String str = "select id,departmentid from hrmresource where workcode = '"+FBuyerID+"'" ;
					rs.executeSql(str);
					if(rs.next()){
						FBuyerID = Util.null2String(rs.getString("id"));
						String departmentid = Util.null2String(rs.getString("departmentid"));
						head.put("FBuyerID", FBuyerID);
//						head.put("cjrbm", departmentid);
						...
						JSONObject jsn1 = null;
						try {
							jsn1 = new JSONObject("{\"return_type\":\"E\",\"return_message\":\""+head.getString("FBuyerID")+"采购员不存在OA中\",\"requestid\":\"0\"}");
						} catch (JSONException e) {
							e.printStackTrace();
							log.writeLog("异常2----"+e.getMessage());
						}
