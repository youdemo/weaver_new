...
public class PostWorkflowInf extends BaseBean {
	...
	public void operateToDo(String requestid, String userid) {
		...
		try {
			...
			String nodename = "";
			String viewtypegd = "";
			String requestlevel = "";// 新增流程紧急程度推送 add by dingji at 20190301
			String sql1 = "select * from workflow_requestbase where requestid=" + requestid;
			rs.executeSql(sql1);
			if (rs.next()) {
			...
			sql1 = "select nodename,isend from workflow_nodebase where id= " + nodeid;
			rs.executeSql(sql1);
			if (rs.next()) {
			...
			String sql2 = "select * from workflow_currentoperator where requestid =" + requestid
					+ (!"".equals(userid) ? " and userid = '" + userid + "'" : "") + "  order by id asc";

			// this.writeLog("PostWorkflowInf.class", "查询待推送的数据SQL:[" + sql2 + "]");

			rs3.executeSql(sql2);
			while (rs3.next()) {
				viewtypegd = Util.null2String(rs3.getString("viewtype"));
...
//				}
				String receivedatetime = Util
						.null2String(rs3.getString("receivedate") + " " + rs3.getString("receivetime"));
				...
				String appurl = "/client.do?method=toDetail&loginid=" + receiver_loginid + "&timestamp=" + timestamp
						+ "&loginTokenFromThird=" + hexSHA1("weaver" + receiver_loginid + timestamp)
						+ "&url=/mobile/plugin/1/view.jsp?detailid=" + requestid;
				...
				writeLog("接收者:" + receiver_loginid + "isremark状态:" + isremark+ "takisremark状态:" + takisremark);
				...
				if ("0".equals(isremark) && "-2".equals(takisremark)) {
					// 已办事宜 往统一待办库发已办消息
					writeLog("意见征询 发起人变已办");
					Map<String, String> dataMap = buildDataMap(syscode, requestid, flowtitle, workflowname, nodename,
							receiver_loginid, requestlevel);
					AnyType2AnyTypeMapEntry[] dataArray = OfsClient.buildDataArray(dataMap);
					AnyType2AnyTypeMapEntry[] resultArray = OfsClient.getClient().processDoneRequestByMap(dataArray);
					// OfsClient.printResultArray(resultArray);
				} else if ("0".equals(isremark) && "0".equals(takisremark)) {
					// 待办事宜 往统一待办库发待办消息
					writeLog("意见征询 待办事宜");
					Map<String, String> dataMap = buildDataMap(syscode, requestid, flowtitle, workflowname, nodename,
							pcurl, appurl, creater_loginid, createdatetime, receiver_loginid, receivedatetime,
							requestlevel);
					AnyType2AnyTypeMapEntry[] dataArray = OfsClient.buildDataArray(dataMap);
					// writeLog("PostWorkflowInf(272)");
					AnyType2AnyTypeMapEntry[] resultArray = OfsClient.getClient().receiveTodoRequestByMap(dataArray);
					// OfsClient.printResultArray(resultArray);
				} else if ("0".equals(isremark)) {
					// 待办事宜
					Map<String, String> dataMap = buildDataMap(syscode, requestid, flowtitle, workflowname, nodename,
							pcurl, appurl, creater_loginid, createdatetime, receiver_loginid, receivedatetime,
							requestlevel);
				...
				if ("1".equals(isremark) || "8".equals(isremark) || "9".equals(isremark)) {
					// this.writeLog("requestid:[" + requestid + "],receiver_loginid:[" +
					// receiver_loginid + "]");
					...
					// 已办事宜 往统一待办库发已办消息
					/*
					 * Map<String,String> dataMap = buildDataMap( syscode, requestid, flowtitle,
					 * workflowname, flownodename, receiver_loginid ); AnyType2AnyTypeMapEntry[]
					 * dataArray = OfsClient.buildDataArray(dataMap); AnyType2AnyTypeMapEntry[]
					 * resultArray = OfsClient.getClient().processDoneRequestByMap(dataArray);
					 * OfsClient.printResultArray(resultArray);
					 */
					Map<String, String> dataMap = buildDataMap(syscode, requestid, flowtitle, workflowname, nodename,
							pcurl, appurl, isremark, viewtype, creater_loginid, createdatetime, receiver_loginid,
							receivedatetime, requestlevel);
					AnyType2AnyTypeMapEntry[] dataArray = OfsClient.buildDataArray(dataMap);
					...
					// + flowtitle + " " + workflowname + " "
					// + nodename + " " + receiver_loginid);
					String viewtype = "0";
					if (!"0".equals(viewtypegd)) {
						viewtype = "1";
					}
					// 办结 往统一待办库发办结消息
					/*
					 ...
					 * workflowname, flownodename, receiver_loginid ); AnyType2AnyTypeMapEntry[]
					 * dataArray = OfsClient.buildDataArray(dataMap); AnyType2AnyTypeMapEntry[]
					 * resultArray = OfsClient.getClient().processOverRequestByMap(dataArray);
					 * OfsClient.printResultArray(resultArray);
					 */
					...
					Map<String, String> dataMap1 = buildDataMap(syscode, requestid, flowtitle, workflowname, nodename,
							receiver_loginid, requestlevel);
...
//							}
	...
	public void operateZXToDo(String requestid, int id) {
		...
		try {
			...
			String sql2 = "select * from workflow_currentoperator where requestid =" + requestid + " and id=" + id
					+ " order by id";
			rs3.executeSql(sql2);
			while (rs3.next()) {
				String receiverid = Util.null2String(rs3.getString("userid"));
				...
				for (String whiteuser : whitelist.split(",")) {
					if (receiver_loginid.equals(whiteuser)) {
						...
						String viewtype = "1";
						Map<String, String> dataMap = buildDataMap(syscode, requestid, flowtitle, workflowname,
								flownodename, pcurl, appurl, isremark, viewtype, creater_loginid, createdatetime,
								receiver_loginid, receivedatetime, requestlevel);
	...
	public void NoReadToRead(String requestid, String userid) {
		...
		try {
			...
			if (rs.getDBType().equals("oracle")) {
				sql2 = "select * from workflow_currentoperator where requestid =" + requestid + " and userid=" + userid
						+ " and NVL(operatedate,'') = '' order by id";
			} else {
				sql2 = "select * from workflow_currentoperator where  requestid =" + requestid + " and userid=" + userid
						+ " and ISNULL(operatedate,'') = '' order by id";
			}
	...
	public void deleteoonlyUserToDo(String requestid, String userid) {
		...
		try {
			...
			AnyType2AnyTypeMapEntry[] dataArray1 = OfsClient.buildDataArray(dataMap1);
			AnyType2AnyTypeMapEntry[] resultArray1 = OfsClient.getClient().deleteUserRequestInfoByMap(dataArray1);
			// OfsClient.printResultArray(resultArray1);
	...
	public void deleteUserToDo(String requestid, String userid) {
		...
		try {
			...
			RecordSet rs3 = new RecordSet();
			String sql2 = "select * from workflow_currentoperator where requestid =" + requestid + "  and userid="
					+ userid + " and islasttimes=1 order by nodeid";
			rs3.executeSql(sql2);
			while (rs3.next()) {
				String receiverid = Util.null2String(rs3.getString("userid"));
				String receiver_loginid = "";
				...
				for (String whiteuser : whitelist.split(",")) {
					if (receiver_loginid.equals(whiteuser)) {
						if ("0".equals(isremark)) {
							...
							receivedatetime = TimeUtil.getCurrentTimeString();
							Map<String, String> dataMap = buildDataMap(syscode, requestid, flowtitle, workflowname,
									flownodename, pcurl, appurl, creater_loginid, createdatetime, receiver_loginid,
									receivedatetime, requestlevel);
							AnyType2AnyTypeMapEntry[] dataArray = OfsClient.buildDataArray(dataMap);
							AnyType2AnyTypeMapEntry[] resultArray = OfsClient.getClient()
									.receiveTodoRequestByMap(dataArray);
							// OfsClient.printResultArray(resultArray);
						}
						if ("1".equals(isremark) || "8".equals(isremark) || "9".equals(isremark)) {
							// 待办事宜 往统一待办库发待办消息
	...
	public void deleteToDo(String requestid) {
		...
		try {
			...
			RecordSet rs3 = new RecordSet();
			String sql2 = "select * from workflow_currentoperator where requestid =" + requestid
					+ "  and islasttimes=1 order by nodeid";
			rs3.executeSql(sql2);
			while (rs3.next()) {
				String receiverid = Util.null2String(rs3.getString("userid"));
				String receiver_loginid = "";
...
//				String viewtype = "1";
				/*
				 * writeLog("接收者:" + receiver_loginid + "isremark状态:" + isremark); Map<String,
				 * String> dataMap = buildDataMap(syscode, requestid, flowtitle, workflowname,
				 * flownodename, pcurl, appurl, isremark, viewtype, creater_loginid,
				 * createdatetime, receiver_loginid, receivedatetime); AnyType2AnyTypeMapEntry[]
				 * dataArray = OfsClient .buildDataArray(dataMap); AnyType2AnyTypeMapEntry[]
				 * resultArray = OfsClient.getClient() .receiveRequestInfoByMap(dataArray);
				 * OfsClient.printResultArray(resultArray);
				 */
				whitelist = receiver_loginid;
				for (String whiteuser : whitelist.split(",")) {
	...
	public Map<String, String> buildDataMap(String syscode, String flowid, String requestname, String workflowname,
			String nodename, String pcurl, String appurl, String creator, String createdatetime, String receiver,
			String receivedatetime, String requestlevel) {
		...
		dataMap.put("requestlevel", requestlevel);
		return dataMap;
	}
	...
	public Map<String, String> buildDataMap(String syscode, String flowid, String requestname, String workflowname,
			String nodename, String receiver, String requestlevel) {
		Map<String, String> dataMap = new HashMap<String, String>();
		dataMap.put("syscode", syscode);
		dataMap.put("flowid", flowid);
		dataMap.put("requestname", requestname);
	...
	public Map<String, String> buildDataMap(String syscode, String flowid, String requestname, String workflowname,
			...
			String createdatetime, String receiver, String receivedatetime, String requestlevel) {
		Map<String, String> dataMap = new HashMap<String, String>();
		dataMap.put("syscode", syscode);
		dataMap.put("flowid", flowid);
		dataMap.put("requestname", requestname);
