package com.wasu.action.finance;
...
/**
 * @Description: 包干外费用报销，增加付款属性
 * @Author: adore
 * @DateTime: 2018-07-20 下午2:10
 * @Version: v1.1
 */
public class CertificateWorkflowAction implements Action {
    ...
    public String execute(RequestInfo info) {
        ...
        writeLog("_-----------------进入包干外action-------------------:" + requestid);
...
//        String f_tabledt = Util.null2String(Prop.getPropValue("FinancialDocuments", "f_tabledt"));
        ...
        if (!"".equals(tableName)) {
            sql = "select " + f_sqr + "," + f_sqbm + "," + f_sy + "," + f_tzjl + "," + f_bxje + "," + f_rzrq + "," +
                    ...
                    " id,lengthb(" + f_sy + ") as syleng,payee,PaymentType,skdwhm,fyssgs from " + tableName + " " +
...
//				 m_sfxyxztzjl = Util.null2String(rs.getString(f_sfxyxztzjl));	//是否需要选择拓展经理field_1442494596
                ...
                fyssgs = Util.null2String(rs.getString("fyssgs"));
            }
        }
        writeLog("fyssgs:"+fyssgs);
        if("1".equals(fyssgs)) {
...
//					resultSet = stmt.getGeneratedKeys();
                    ...
                    String dt_sql = " select * from " + detailtablename + " where mainid = " + mainID + " order by id ";
                    writeLog("dt_sql:" + dt_sql);
                    ...
                    for (String subj : newSubj) {
                        ...
                        while (rs.next()) {
                            String tmp_need_kmdm = ""; // 循环内要用局部变量
                            String subID = "";//科目ID
...
//						}
                            ...
                            if (rsds.next()) {
                                ...
                                if ("".equals(tmp_need_kmid) && tmp_need_kmid != null) {
