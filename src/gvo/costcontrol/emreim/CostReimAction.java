 ...
 *
 */
public class CostReimAction implements Action {
	...
	public String execute(RequestInfo info) {
		...
		while (rs.next()) {
			...
			String FYKMBM = Util.null2String(rs.getString("kmbm"));// 费用科目编码
			String EXPENSEAMT = Util.null2String(rs.getString("bhsje"));// 费用未税金额
			String TAX = Util.null2String(rs.getString("se"));// 税额
			String fphm = Util.null2String(rs.getString("zzsfph"));// 发票号码
		...
		while (rs.next()) {
			...
			String PAYAMT = Util.null2String(rs.getString("bccxyfkje"));// 本次冲销金额
			String LWFLOWNO = Util.null2String(rs.getString("yfdh"));// 借款单号
			String yflx =  Util.null2String(rs.getString("yflx"));
			GENERALACCOUNT = yflx;
