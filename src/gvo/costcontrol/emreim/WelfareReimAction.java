 ...
 *
 */
public class WelfareReimAction  implements Action {
	
	public String execute(RequestInfo info) {
		BaseBean log = new BaseBean();
		log.writeLog("进入福利费报销流程WelfareReimAction————————");
		...
		if (rs.next()) {
			...
			FLOWNO = Util.null2String(rs.getString("djbh"));
		...
		rs.executeSql(sql);
		if(rs.next()){
			REQCODE = Util.null2String(rs.getString("ygbh3"));
		}
		// 查询明细表1
		...
		while (rs.next()) {
			...
			String EXPENSEAMT = Util.null2String(rs.getString("fywsje"));// 费用未税金额
			String TAX = Util.null2String(rs.getString("zpse"));// 税额
			String fphm = Util.null2String(rs.getString("fphm1"));// 发票号码
			try {
		...
		while (rs.next()) {
			...
			String PAYAMT = Util.null2String(rs.getString("mx2bccxje"));// 本次冲销金额
			String LWFLOWNO = Util.null2String(rs.getString("mx2jkdh"));// 借款单号
