...
import gvo.purchase.updatestatus.StateXmlUtil;
import gvo.purchase.updatestatus.UpdatePRApproval;
...
public class UpdatePRStatusAction implements Action {
	/**
	 * 采购申请修改更新审批中状态
	 * 
	 * @author Tommy
	...
	public String execute(RequestInfo info) {
		log.writeLog("进入采购申请修改状态更新 UpdateStatusAction——————");
		String workflowID = info.getWorkflowid();
		String requestid = info.getRequestid();
		RecordSet rs = new RecordSet();
		String tableName = "";
		String PRNO = "";// 采购申请单号
		String STATUS = "1";//审批标识
		String sql = "";
		...
		if (!"".equals(tableName)) {
			...
			rs.execute(sql);
			if (rs.next()) {
				PRNO = Util.null2String(rs.getString("pqNo"));
			}
