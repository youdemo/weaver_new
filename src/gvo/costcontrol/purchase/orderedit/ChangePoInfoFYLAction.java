...
public class ChangePoInfoFYLAction implements Action {
	/**
	 * 采购订单修改接口
	 * 
	 * @author tangjianyong
	...
	public String execute(RequestInfo info) {
		log.writeLog("进入费用类采购订单修改流程 ChangePoInfoFYLAction————————————");
		...
		if (rs.next()) {
			...
			while (rs.next()) {
				...
				String NEWNUM = Util.null2String(rs.getString("xghsl"));// 修改后数量
				String NEWPRICE = Util.null2String(rs.getString("xghdj"));// 修改后单价
				String NEWTAXCODE = Util.null2String(rs.getString("newtaxcode"));// 修改税码
				String NEWOUTDATE = Util.null2String(rs.getString("xghjq"));// 修改后交期
