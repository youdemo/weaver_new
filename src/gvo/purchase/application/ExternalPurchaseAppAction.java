...
public class ExternalPurchaseAppAction implements Action {
	/**
	 * 委外加工采购申请新增
	 * 
	 * @author Tommy
	...
	public String execute(RequestInfo info) {
		log.writeLog("进入委外采购申请流程ExternalPurchaseAppAction——————");
		...
		if (!"".equals(tableName)) {
			...
			while(rs.next()){
				...
				try {
					jo.put("BNFPO", Util.null2String(rs.getString("BNFPO")));//项目（OAID)
					jo.put("POSNR", Util.null2String(rs.getString("POSNR")));//组件项目
					jo.put("IDNRK", Util.null2String(rs.getString("IDNRK")));//组件物料号
					jo.put("MAKTX", Util.null2String(rs.getString("MAKTX")));//组件物料描述
					jo.put("MENGE", Util.null2String(rs.getString("MENGE")));//需求数量
					jo.put("MEINS", Util.null2String(rs.getString("MEINS")));//组件单位
					jo.put("BANFN", Util.null2String(rs.getString("BANFN")));//采购申请号
					jo.put("RSNUM", Util.null2String(rs.getString("RSNUM")));//关联预留号
					jsonArray2.put(jo);
				} catch (JSONException e) {
			...
			try {
				...
				head.put("CHILD_CreatePR_SAP_2_LIST", jsonArray2);
