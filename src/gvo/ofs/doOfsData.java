...
public class doOfsData extends BaseCronJob{
	...
	public void doUnPushData(){
		...
		String requestid="";
		String sql="select requestid from workflow_requestbase  a where a.currentnodetype<3  and requestid not in(select distinct flowid from ofs_todo_data@dblink_oa) and a.creater <> 1 and a.workflowid not in(644,1) and a.createdate>='2018-01-01'";
		rs.executeSql(sql);
		while(rs.next()){
