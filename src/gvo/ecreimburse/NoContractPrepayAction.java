 ...
 **/
public class NoContractPrepayAction implements Action {

	BaseBean log = new BaseBean();

	public String execute(RequestInfo info) {
		log.writeLog("进入对公无合同预付款 NoContractPrepayAction——————");
		String workflowID = info.getRequestid();
		String requestid = info.getRequestid();
		RecordSet rs = new RecordSet();
		String sql = "";
		String tableName = info.getRequestManager().getBillTableName();// 主表
		...
		if (!"".equals("tableName")) {
			...
			try {
				...
				while (rs.next()) {
					...
					String ZZBS = Util.null2String(rs.getString("yflx1"));// 预付类型
