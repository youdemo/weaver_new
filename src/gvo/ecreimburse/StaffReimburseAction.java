...
/**
 * EC与资金V0-005员工费用报销申请流程
 * 
 * @author daisy
 * @version 1.0 2017-11-22
 **/
public class StaffReimburseAction implements Action {

	BaseBean log = new BaseBean();

	public String execute(RequestInfo info) {
		log.writeLog("进入员工费用报销申请流程 StaffReimburseAction——————");
		...
		sql = " Select tablename From Workflow_bill Where id in ( Select formid From workflow_base Where id= "
				+ workflowID + ")";
		rs.execute(sql);
		log.writeLog("开始————————sql" + sql);
		if (rs.next()) {
			tableName = Util.null2String(rs.getString("tablename"));
			log.writeLog("开始————————tableName" + tableName);
		}
