...
/**
 * EC与资金V0-011培训费报销申请流程
 * 
 * @author daisy
 * @version 1.0 2017-11-22
 **/
public class TrainFeeAction implements Action {
	BaseBean log = new BaseBean();

	public String execute(RequestInfo info) {
		log.writeLog("进入培训费报销申请 TrainFeeAction——————");
		...
		if (!"".equals(tableName)) {
			...
			if (rs.next()) {
				...
				ABS = Util.null2String(rs.getString("fybxsm"));
				if (ABS.length() >= 50) {
				...
				reqsub = Util.null2String(rs.getString("reqsub"));
			}
			...
			if("82".equals(reqsub) || "83".equals(reqsub) || "84".equals(reqsub)) {
				try {
					...
					String sql_update = "update " + tableName + " set status='"
							+ sign + "',message='" + mess + "' where requestid="
							+ requestid;
					rs.execute(sql_update);
					if ("F".equals(sign)) {
						// 调用异常 返回错误信息
						info.getRequestManager().setMessageid(
